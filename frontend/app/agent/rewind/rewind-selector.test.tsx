...
describe("RewindSelector", () => {
    ...
    ])("focuses the panel and keeps Escape available in the $name state", async ({ props: stateProps }) => {
        const onClose = vi.fn();
        renderSelector({ ...stateProps, onClose });

        const listbox = screen.getByRole("listbox", { name: "Rewind points" });
    ...
    it("renders nothing while closed and distinct loading, error, empty, and no-match states", () => {
        ...
        fireEvent.change(screen.getByRole("textbox", { name: "Search rewind points" }), {
    ...
    it("filters by prompt preview and resets to the most recent eligible match", async () => {
        ...
        const search = screen.getByRole("textbox", { name: "Search rewind points" });
    ...
    it("does not steal focus when a search transitions from no matches back to matches", async () => {
        ...
        await waitFor(() =>
            expect(document.activeElement).toBe(screen.getByRole("listbox", { name: "Rewind points" }))
        );
        const search = screen.getByRole("textbox", { name: "Search rewind points" });
        search.focus();
    ...
    it("does not commit stale points while loading or showing an error", () => {
        ...
        fireEvent.keyDown(screen.getByRole("listbox", { name: "Rewind points" }), { key: "Enter" });
    ...
    it("uses unique option ids across simultaneous agent surfaces", async () => {
        ...
        const [first, second] = screen.getAllByRole("listbox", { name: "Rewind points" });
        await waitFor(() => expect(first.getAttribute("aria-activedescendant")).toBeTruthy());
        const firstOptionId = first.getAttribute("aria-activedescendant");
        const secondOptionId = second.getAttribute("aria-activedescendant");
