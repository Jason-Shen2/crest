...
const FitDebounceMs = 8;
const PtyResizeDebounceMs = 256;
const SnapshotScrollbackCap = 5_000;
const McrBgActive = 4.5;
const McrBgInactive = 1;
const WebglRecoveryDelayMs = 250;
// Below this a re-shown slot is fresh enough to trust; above it, repaint on
// unhide to defeat silent GPU/context staleness.
const SlotStaleMs = 10_000;
const WebglReapGraceMs = 30_000;
const SlotReapGraceMs = 45_000;
const IdleSlotsKeepWarm = 1;
...
function getRecycler(): HTMLDivElement {
    ...
    el.setAttribute("data-xterm-recycler", "");
    el.style.cssText =
...
function termOptions() {
    return {
        ...
        fontSize: Math.max(4, Math.round(getTermFontSize())),
        theme: buildTerminalTheme(),
        cursorBlink: false,
        cursorStyle: "bar" as const,
        cursorInactiveStyle: "outline" as const,
        scrollback: getTermScrollback(),
        allowProposedApi: true,
        minimumContrastRatio: McrBgInactive,
    };
}

export function applyBackgroundActive(active: boolean): void {
    const value = active ? McrBgActive : McrBgInactive;
    for (const slot of slots) {
...
function createSlot(): Slot {
    ...
    term.loadAddon(new WebLinksAddon((_e, uri) => getApi().openExternal(uri)));
    ...
    term.attachCustomKeyEventHandler((event) => {
        ...
        const deleteSeq = terminalDeleteSequence(event, { isMac: IsMac });
        if (deleteSeq) {
            event.preventDefault();
...
export function evictionScore(s: Slot): number {
...
export function acquireSlot(params: AcquireParams): Slot {
    ...
    if (pick.slot.retainedLeafId !== null && pick.slot.retainedLeafId !== params.leafId) {
...
function setupResizeObserver(slot: Slot, p: AcquireParams): void {
    ...
    slot.observer = new ResizeObserver(() => {
        ...
        slot.fitTimer = setTimeout(() => {
            ...
            slot.fitAddon.fit();
            if (slot.ptyTimer) clearTimeout(slot.ptyTimer);
            slot.ptyTimer = setTimeout(flushPty, PtyResizeDebounceMs);
        }, FitDebounceMs);
    });
...
export function snapshotScrollbackCap(): number {
    return Math.min(SnapshotScrollbackCap, getTermScrollback());
}

function serializeSlot(slot: Slot): SerializeOutput {
    let snapshot: string | null = null;
    try {
        snapshot = slot.serializeAddon.serialize({ scrollback: snapshotScrollbackCap() });
    } catch (e) {
        console.warn("[xterm-pool] serialize failed:", e);
    }
    return {
        snapshot,
        cols: slot.term.cols,
...
function reapIdleSlot(slot: Slot): void {
    ...
    if (idle.length <= IdleSlotsKeepWarm) return;
    idle.sort((a, b) => a.lastUsedAt - b.lastUsedAt);
    const surplus = idle.slice(0, idle.length - IdleSlotsKeepWarm);
...
function disposeSlot(slot: Slot): void {
    ...
    try {
        ...
        console.warn("[xterm-pool] slot dispose failed:", e);
    }
    slot.host.remove();
...
function attachWebgl(slot: Slot): void {
    ...
    try {
        ...
        console.warn("[xterm-webgl] unavailable:", e);
    }
}
...
function disposeSlotWebgl(slot: Slot): void {
    ...
    try {
        ...
        console.warn("[xterm-webgl] dispose failed:", e);
    }
    try {
        const r = (addon as unknown as { _renderer?: Record<string, unknown> | null })._renderer;
        if (r) {
            r._canvas = null;
            r._gl = null;
            r._charAtlas = null;
        ...
        (addon as unknown as { _renderer?: unknown; _renderService?: unknown })._renderer = null;
        (addon as unknown as { _renderer?: unknown; _renderService?: unknown })._renderService = null;
    } catch {}
    slot.webglAddon = null;
}
...
export function refreshLeafSlot(leafId: number): void {
    ...
    cancelWebglReap(slot);
    unparkSlotHost(slot);
    if (isTermWebglEnabled() && !slot.webglAddon) {
        attachWebgl(slot);
    }
...
export function discardRetainedSlot(leafId: number): void {
    const slot = slots.find((s) => s.currentLeafId === null && s.retainedLeafId === leafId);
    if (!slot) return;
    discardRetention(slot);
    slot.term.clear();
    slot.tuiWheel?.resetTerminal();
