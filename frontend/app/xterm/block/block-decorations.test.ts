...
import { BlockDecorations } from "./block-decorations";
...
describe("BlockDecorations — cmdblock:row enrichment (applyRowMeta)", () => {
    function withFinishedBlock(exit = "0") {
        const fake = makeFakeTerm();
        const deco = new BlockDecorations(fake.term);
        fake.setLine(3);
        fake.emit("C");
    ...
    it("attaches the Go-side cmd to a time-aligned entry", () => {
        const { deco } = withFinishedBlock();
        const applied = deco.applyRowMeta({ cmd: "ls -al", startedAt: Date.now() });
        expect(applied).toBe(true);
        expect(deco.getBlocks()[0].command).toBe("ls -al");
    });

    it("fills a missing exit code but never flips one the OSC already set", () => {
        const { deco, emit, setLine } = withFinishedBlock("1");
        expect(deco.applyRowMeta({ exitCode: 0, finishedAt: Date.now() })).toBe(false);
        expect(deco.getBlocks()[0].exitCode).toBe(1);

        setLine(10);
        emit("C");
        setLine(12);
        emit("D"); // no code payload → exitCode null
        expect(deco.applyRowMeta({ exitCode: 7, finishedAt: Date.now() })).toBe(true);
        expect(deco.getBlocks()[1].exitCode).toBe(7);
    });

    it("recomputes finishedAt from the persisted duration", () => {
        const { deco } = withFinishedBlock();
        deco.applyRowMeta({ durationMs: 1234, finishedAt: Date.now() });
        const b = deco.getBlocks()[0];
        expect(b.finishedAt - b.startedAt).toBe(1234);
    });

    it("silently ignores rows outside the alignment window", () => {
        const { deco } = withFinishedBlock();
        const applied = deco.applyRowMeta({ cmd: "stale", startedAt: Date.now() - 60_000 });
        expect(applied).toBe(false);
        expect(deco.getBlocks()[0].command).toBe("");
    });

    it("ignores rows when there is nothing to match", () => {
        const { term } = makeFakeTerm();
        const deco = new BlockDecorations(term);
        expect(deco.applyRowMeta({ cmd: "ls", startedAt: Date.now() })).toBe(false);
    });

    it("labels the live block from a running row", () => {
        const { term, emit, setLine } = makeFakeTerm();
        const deco = new BlockDecorations(term);
        setLine(1);
        emit("C");
        expect(deco.applyRowMeta({ cmd: "sleep 5", running: true, startedAt: Date.now() })).toBe(true);
        emit("D;0");
        expect(deco.getBlocks()[0].command).toBe("sleep 5");
    });
});
