...
import type { ModeEvent, Osc133Marker } from "./block/mode-machine";
import { writeTerminalClipboard } from "./terminal-clipboard";

const MaxOsc52ClipboardBytes = 1024 * 1024;
...
export function registerCwdHandler(
    ...
    opts?: CwdHandlerOptions
): () => void {
    const isWin = opts?.isWindowsPlatform ?? isWindows;
    const d = term.parser.registerOscHandler(7, (data) => {
        ...
        const cwd = parseOsc7(data, isWin());
        if (cwd) onCwd(cwd);
        return true;
    });
    return () => d.dispose();
...
export function registerPromptTracker(
    ...
    onModeEvent?: (event: ModeEvent) => void
): PromptTracker {
...
function osc133Event(marker: Osc133Marker): ModeEvent {
    return { type: "osc133", marker };
}
...
function parseOsc7(data: string, isWin: boolean): string | null {
...
function parseOsc52Clipboard(data: string): string | null {
    ...
    if (encoded.length > Math.ceil((MaxOsc52ClipboardBytes * 4) / 3) + 4) {
        return null;
    }
    ...
    try {
        const bytes = base64ToArray(compact);
        if (bytes.byteLength > MaxOsc52ClipboardBytes) return null;
