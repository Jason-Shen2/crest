...
import { PlatformWindows, setPlatform } from "@/util/platformutil";
import type { Terminal } from "@xterm/xterm";
import { describe, expect, it, vi } from "vitest";
import type { ModeEvent } from "./block/mode-machine";
import {
 ...
 * enough to register OSC handlers and invoke them with crafted payloads. The
 * OSC handler signature is `(data: string) => boolean | Promise<boolean>`.
 */
...
describe("OSC 7 cwd handler — gated by OSC 133 in-command state", () => {
    ...
    it("honors an injected platform check over the global platform", () => {
        const { term, handlers } = makeFakeTerm();
        const onCwd = vi.fn();
        registerCwdHandler(term, onCwd, undefined, { isWindowsPlatform: () => false });

        // With the git-bash mapping off, a /c/... path passes through untouched.
        handlers.get(7)?.("file:///c/Users/leo/project");
        expect(onCwd).toHaveBeenCalledWith("/c/Users/leo/project");
    });
});
...
describe("OSC 133 mode-machine event dispatch", () => {
    it("dispatches an osc133 mode event for every A/B/C/D marker", () => {
        const { term, handlers } = makeFakeTerm();
        const events: ModeEvent[] = [];
        registerPromptTracker(term, undefined, undefined, (event) => events.push(event));
        ...
        expect(events).toEqual([
            { type: "osc133", marker: "A" },
            { type: "osc133", marker: "B" },
            { type: "osc133", marker: "C" },
            { type: "osc133", marker: "D" },
        ]);
    });
