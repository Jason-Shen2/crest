...
type UserTurnTerminalReason = "superseded" | "agent_end" | "aborted" | "preparation_failed" | "provider_failed";

interface UserTurnBoundary {
    token: string;
    message: UserMessage;
    userEntryId?: string;
    committedDispatched: boolean;
...
> {
    ...
    private userTurnBoundaries = new WeakMap<UserMessage, UserTurnBoundary>();
    private activeUserTurn?: UserTurnBoundary;
    private activeUserTurnFailureReason?: Extract<UserTurnTerminalReason, "aborted" | "provider_failed">;
    private userTurnFinalizationSignal = new AbortController().signal;
    private userTurnFinalizationPending = false;
    ...
    private async beginUserTurn(userMessage: UserMessage, signal?: AbortSignal): Promise<UserTurnBoundary> {
        const existing = this.userTurnBoundaries.get(userMessage);
        if (existing && !existing.terminal) return existing;
        if (this.activeUserTurn && !this.activeUserTurn.terminal) {
            await this.terminalizeUserTurn(this.activeUserTurn, "superseded");
        }
        const boundary: UserTurnBoundary = {
            token: `boundary-${++this.userTurnCounter}`,
            message: userMessage,
            committedDispatched: false,
            terminalDispatched: false,
            committed: false,
            terminal: false,
        };
        this.userTurnBoundaries.set(userMessage, boundary);
        this.activeUserTurn = boundary;
        this.activeUserTurnFailureReason = undefined;
        try {
            await this.emitOwn(
                {
            ...
            try {
                await this.terminalizeUserTurn(boundary, "preparation_failed");
            } catch (terminalError) {
                throw new AggregateError(
                    [toError(error), toError(terminalError)],
                    "User turn preparation hooks failed"
                );
            }
            throw error;
        }
    ...
    private throwUserTurnFinalizationError(boundary: UserTurnBoundary): void {
        if (!boundary.finalizationError) return;
        throw boundary.finalizationError;
    }

    private failUserTurnFinalization(boundary: UserTurnBoundary, error: unknown): never {
        const finalizationError = toError(error);
        boundary.finalizationError = finalizationError;
        this.userTurnFinalizationPending = true;
        throw finalizationError;
    }

    private async emitUserTurnCommitted(boundary: UserTurnBoundary): Promise<void> {
        if (boundary.committed) return;
        this.throwUserTurnFinalizationError(boundary);
        if (!boundary.userEntryId) {
            throw new AgentHarnessError("invalid_state", "Cannot commit a user turn without a durable entry");
        }
        if (boundary.committedDispatched) {
            throw new AgentHarnessError("invalid_state", "User turn committed event did not settle");
        }
        boundary.committedDispatched = true;
        try {
            await this.emitOwn(
                {
                    type: "session_user_turn_committed",
                    boundaryToken: boundary.token,
                    userEntryId: boundary.userEntryId,
                },
                this.userTurnFinalizationSignal
            );
            boundary.committed = true;
            this.userTurnFinalizationPending = false;
        } catch (error) {
            this.failUserTurnFinalization(boundary, error);
        }
    }

    private async commitUserTurn(userMessage: UserMessage, userEntryId: string): Promise<void> {
        const boundary = this.userTurnBoundaries.get(userMessage) ?? (await this.beginUserTurn(userMessage));
        if (boundary.userEntryId && boundary.userEntryId !== userEntryId) {
            throw new AgentHarnessError("invalid_state", "User turn committed with a different durable entry");
        }
        boundary.userEntryId = userEntryId;
        if (boundary.committed) return;
        await this.emitUserTurnCommitted(boundary);
    }

    private async terminalizeUserTurn(boundary: UserTurnBoundary, reason: UserTurnTerminalReason): Promise<void> {
        if (boundary.terminal) return;
        this.throwUserTurnFinalizationError(boundary);
        if (boundary.userEntryId && !boundary.committed) {
            await this.emitUserTurnCommitted(boundary);
        }
        this.throwUserTurnFinalizationError(boundary);
        if (boundary.terminalDispatched) {
            throw new AgentHarnessError("invalid_state", "User turn terminal event did not settle");
        }
        boundary.terminalDispatched = true;
        try {
            await this.emitOwn(
                {
            ...
            this.userTurnFinalizationPending = false;
            if (this.activeUserTurn === boundary) {
    ...
    private async terminalizeActiveUserTurn(reason: UserTurnTerminalReason): Promise<void> {
        if (!this.activeUserTurn) return;
        await this.terminalizeUserTurn(
            this.activeUserTurn,
            this.activeUserTurn.userEntryId ? reason : "preparation_failed"
        );
    }

    private async failUserTurnPreparation(userMessage: UserMessage, signal?: AbortSignal): Promise<void> {
        const boundary = this.userTurnBoundaries.get(userMessage);
        if (!boundary || boundary.terminal) return;
        await this.terminalizeUserTurn(
            boundary,
            boundary.userEntryId ? (signal?.aborted ? "aborted" : "provider_failed") : "preparation_failed"
        );
    }

    private releaseTurnPhaseAfterFailure(): void {
        if (this.userTurnFinalizationPending) return;
        this.phase = "idle";
    }
    ...
    private async prepareUserMessage(
        ...
        return await prepare({
            ...
            transformProviderRequest: async () => {
                ...
                return await this.emitBeforeProviderRequest(turnState.model, turnState.sessionId, snapshotOptions);
            },
                    ...
                    if (this.preparedProviderPayloads.length > 0) {
                        ...
                        if (expectedUserEntryId == null || prepared.userEntryId !== expectedUserEntryId) {
                            ...
                            throw new AgentHarnessError(
                                "invalid_state",
                                "Prepared provider payload is out of sequence"
                            );
                        }
        ...
        return {
            ...
            convertToLlm: (messages) => {
                ...
                const sourceEntryIds =
                    ...
                    alignMessageEntryIds(current.messages, current.messageEntryIds, messages, this.messageEntryIds);
            ...
            beforeUserMessage: async (message) => {
                await this.beginUserTurn(message, signal);
            },
            ...
            prepareFollowUpTurn: async () => {
                ...
                try {
                    await this.beginUserTurn(userMessage, signal);
                    ...
                    const nextTurnState = await this.createTurnState();
                    setTurnState(nextTurnState);
                    const prepare = typeof activatedPrepare === "function" ? activatedPrepare : drain.preparations[0];
                    if (!prepare) {
                    ...
                    await this.commitUserTurn(userMessage, prepared.userEntryId);
                    if (signal?.aborted) {
                        throw new AgentHarnessError("invalid_state", "Turn preparation was aborted");
                    }
                    this.inFlightFollowUpDrain = undefined;
                    ...
                    try {
                        await this.failUserTurnPreparation(userMessage, signal);
                    } catch (caught) {
                    ...
                    if (lifecycleError) {
                        throw new AggregateError(
                            [toError(terminal ? (error.cause ?? error) : error), toError(lifecycleError)],
                            "Follow-up preparation and lifecycle finalization failed"
                        );
                    }
    ...
    private async handleAgentEvent(event: AgentEvent, signal?: AbortSignal): Promise<void> {
        if (event.type === "message_end") {
            ...
            if (event.message.role === "user") {
                await this.commitUserTurn(event.message, entryId);
            } else if (event.message.role === "assistant") {
                if (event.message.stopReason === "aborted") {
                    this.activeUserTurnFailureReason = "aborted";
                } else if (event.message.stopReason === "error") {
                    this.activeUserTurnFailureReason = "provider_failed";
                }
            }
            this.messageEntryIds.set(event.message as object, entryId);
            await this.emitAny({ ...event, entryId }, signal);
        ...
        if (event.type === "agent_end") {
            await this.flushPendingSessionWrites();
            await this.terminalizeActiveUserTurn(this.activeUserTurnFailureReason ?? "agent_end");
            this.userTurnFinalizationPending = true;
            await this.emitAny(event, signal);
            await this.emitOwn(
    ...
    private async executeTurn(
        ...
        try {
            if (options?.prepare) {
                await this.beginUserTurn(userMessage, abortController.signal);
                preparedTurn = await this.prepareUserMessage(
                    {
                        ...effectiveTurnState,
                        messages: [...turnState.messages, ...messages],
                    },
                ...
                await this.commitUserTurn(userMessage, preparedTurn.userEntryId);
            }
            ...
            let lifecycleError: unknown;
            try {
                await this.failUserTurnPreparation(userMessage, abortController.signal);
            } catch (caught) {
                lifecycleError = caught;
            }
            ...
            if (lifecycleError) {
                throw new AggregateError(
                    [toError(error), toError(lifecycleError)],
                    "Turn preparation and lifecycle finalization failed"
                );
            }
            throw error;
        }
        ...
        const runResultPromise = (async () => {
            try {
                return await runAgentLoop(
                    ...
                    this.createContext(
                        effectiveTurnState,
                        joinSystemPrompt(effectiveTurnState.systemPrompt, preparedTurn?.systemPromptSuffix ?? "")
                    ),
        ...
        try {
            ...
            try {
                try {
                    await this.terminalizeActiveUserTurn(
                        abortController.signal.aborted ? "aborted" : "provider_failed"
                    );
                } finally {
                    await this.flushPendingSessionWrites();
                }
        ...
        while (this.followUpQueue.length > 0) {
            ...
            try {
                ...
                if (lifecycleError) {
                    throw new AggregateError(
                        [toError(error), toError(lifecycleError)],
                        "Follow-up activation and lifecycle finalization failed"
                    );
                }
                throw error;
            }
    ...
    async prompt(text: string, options?: AgentHarnessPromptOptions): Promise<AssistantMessage> {
        ...
        try {
            ...
            this.releaseTurnPhaseAfterFailure();
    ...
    async promptReturningEntryId(
        ...
        try {
            ...
            const userEntry = branch.find(
                (e) =>
                    e.type === "message" && !baseEntryIds.has(e.id) && (e.message as { role?: string }).role === "user"
            );
            if (!userEntry) {
