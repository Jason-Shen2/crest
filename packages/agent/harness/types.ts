 ...
 *
 * Crest-local change vs upstream pi: we flatten the discriminated union to a single
 * shape with optional `value` and `error` fields. The original
 *   `{ ok: true; value } | { ok: false; error }`
 * relies on TS narrowing on `.ok`, which is unreliable under crest's
 * `strict: false` tsconfig — call sites that do
 *   `if (r.ok) ... else r.error`
 * fail to compile because TS can't drop the `{ ok: true }` branch in the else.
 * The flat shape costs us one runtime invariant (`value` / `error` are mutually
 * exclusive at construction time via ok() / err() helpers below) but eliminates
 * dozens of false-positive narrowing errors across the harness.
 */
export type Result<TValue, TError> = { ok: boolean; value?: TValue; error?: TError };
...
> =
    ...
    | AbortEvent
    | SettledEvent
    | SessionBeforeUserTurnEvent
    | SessionUserTurnCommittedEvent
...
export type AgentHarnessEventResultMap = {
    ...
    abort: undefined;
    settled: undefined;
    session_before_user_turn: undefined;
    session_user_turn_committed: undefined;
    session_user_turn_terminal: undefined;
};
