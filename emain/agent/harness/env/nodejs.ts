...
export class NodeExecutionEnv implements ExecutionEnv {
	...
	async exec(
		...
		// Cast: crest flattened Result<T,E> so TS can't propagate the
		// {ok:false, error:E} branch directly into a Result<U,E> return.
		// Runtime semantics are identical.
		if (!shellConfig.ok) return shellConfig as unknown as Result<{ stdout: string; stderr: string; exitCode: number }, ExecutionError>;
