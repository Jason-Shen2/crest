...
export function parseHeaderLine(line: string, filePath: string): SessionHeader {
	...
	return {
		...
		cwd: parsed.cwd,
		// parsed.parentSession is `unknown` for TS but already validated as
		// (string | undefined) above. crest's `strict: false` doesn't narrow
		// the index access, hence the cast.
