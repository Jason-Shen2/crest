...
export function parseContextDeliveryScope(value: unknown): ContextDeliveryScope {
    if (value === "message" || value === "conversation") return value;
    return invalid("delivery scope must be message or conversation");
}

export function parseContextRepresentation(value: unknown): ContextRepresentation {
    if (value === "full" || value === "summary") return value;
    return invalid("representation must be full or summary");
}
...
function validateProvenance(value: unknown): ContextProvenance {
    ...
    const sourceMessageEntryIds = value.sourceMessageEntryIds.map((entryId) =>
        requiredString(entryId, "sourceMessageEntryIds")
    );
...
function validateSnapshotBlock(value: unknown): ContextSnapshotBlock {
    ...
    if (value.type === "image_omitted") {
        ...
        return {
            type: "image_omitted",
            mimeType: requiredString(value.mimeType, "mimeType"),
...
function validateSnapshotMessage(value: unknown): ContextSnapshotMessage {
    ...
    if (value.role !== "user" && value.role !== "assistant" && value.role !== "tool_result")
        invalid("snapshot message role is invalid");
...
export function validateContextArtifact(value: unknown): ContextArtifact {
    ...
    if (
        typeof value.canonicalByteLength !== "number" ||
        !Number.isFinite(value.canonicalByteLength) ||
...
export function validateContextAttachmentData(value: unknown): ContextAttachmentData {
    ...
    const deliveryScope = parseContextDeliveryScope(value.deliveryScope);
    if (
        typeof value.selectionOrder !== "number" ||
        !Number.isInteger(value.selectionOrder) ||
...
export function decodeContextArtifact(value: unknown): ContextDecodeResult<ContextArtifact> {
    if (isRecord(value) && value.schemaVersion !== 1)
        return { diagnostic: "context artifact schemaVersion is unsupported" };
    try {
...
export function decodeContextAttachmentData(value: unknown): ContextDecodeResult<ContextAttachmentData> {
    if (isRecord(value) && value.schemaVersion !== 1)
        return { diagnostic: "context attachment schemaVersion is unsupported" };
    try {
...
function parseRenderedRepresentation(value: unknown): ContextRenderedRepresentation {
    if (value === "full" || value === "summary" || value === "attention") return value;
...
function validateProjectionItem(value: unknown): ContextProjectionItemReport {
    ...
    if (reason !== "selected" && reason !== "already_present") invalid("projection item reason is invalid");
    ...
    return {
        ...
        deliveryScope: parseContextDeliveryScope(value.deliveryScope),
        requestedRepresentation:
            value.requestedRepresentation == null
                ? undefined
                : parseContextRepresentation(value.requestedRepresentation),
...
export function validateContextProjectionReport(value: unknown): ContextProjectionReport {
    ...
    if (countAccuracy !== "exact" && countAccuracy !== "conservative_upper_bound" && countAccuracy !== "estimated") {
        invalid("projection countAccuracy is invalid");
    }
    ...
    return {
        ...
        overlaySha256: (() => {
            ...
            if (!Sha256Pattern.test(overlaySha256))
                invalid("projection overlaySha256 must be a lowercase SHA-256 hash");
...
export function decodeContextProjectionReport(value: unknown): ContextDecodeResult<ContextProjectionReport> {
    if (isRecord(value) && value.schemaVersion !== 1)
        return { diagnostic: "context projection report schemaVersion is unsupported" };
    try {
