...
[emain/agent/types.ts](file:///Users/bytedance/Documents/crest/.worktrees/agent-timeline-single-source/emain/agent/types.ts) 的 `AgentEvent.message_end` 变体新增可选字段 `entryId?: string`。
...
这是"session 是单一真相源"的直接体现：**身份不是谁 mint 的，是 session 写入时自然产生的**，事件流只负责把它带出来。
