...
> 专职 PTY 周旋的委派 subagent。主 agent 不直接跟长运行 / 交互式 PTY 命令打交道——它下达一个自然语言任务，把 PTY 的脏活（写输入、读输出、判断状态、容错重试）全部委派给 CLI subagent，最后只收回一段自然语言总结。
>
> 本文档是**开发依据**。设计参考 Warp 的 `triggers_server_subagent` 路由模式，复用 crest 现有 [`AgentHarness`](file:///Users/bytedance/Documents/crest/emain/agent/harness/agent-harness.ts) 基础设施。整体双形态架构的可视化说明见 [`agent-dual-mode-design.html`](file:///Users/bytedance/Documents/crest/docs/agent-dual-mode-design.html) §8；Warp 分析见 [`warp-agent-analysis.md`](file:///Users/bytedance/Documents/crest/docs/warp-agent-analysis.md)。

---

## 1. 定位与边界

主 agent 的 shell 能力已经存在（[`bash` 工具](file:///Users/bytedance/Documents/crest/emain/agent/tools/bash.ts#L131-L261)），它适合**会自然结束的一次性命令**。CLI subagent 只在命令**不会自行退出**（长运行 / 交互式）时介入。

| 命令类型 | 例子 | 谁执行 | 进主 agent context | 使用工具 |
| --- | --- | --- | --- | --- |
| 一次性（会自然结束） | `ls`、`git status`、`npm run build` | 主 agent 亲自跑 | output 原文进 context（agent 要读 file:line） | [`bash`](file:///Users/bytedance/Documents/crest/emain/agent/tools/bash.ts#L131-L261) |
...
CLI subagent **只在主 agent 主动委派时启动**。用户在 Agent Conversation 里手动运行 PTY 命令，不自动启动 CLI subagent。

| 场景 | 是否启动 CLI subagent | 处理 |
| --- | --- | --- |
| 主 agent 决定执行长运行 / 交互式命令 | ✅ 是 | 主 agent 调 `spawn_cli_agent`，CLI subagent 负责启动/观察/交互/总结 |
| 主 agent 执行一次性命令 | ❌ 否 | 继续使用 [`bash`](file:///Users/bytedance/Documents/crest/emain/agent/tools/bash.ts#L131-L261) |
| 用户手动运行 PTY 命令 | ❌ 否 | 用户自己是操作者，只记录命令与输出，不自动接管 |
| 用户要求 agent 分析/接管已有 PTY | ✅ 可选 | 需要明确用户意图或确认，可通过 `attach_to_block_id` 扩展接管 |

这个边界避免权限混乱：用户手动输入命令时，agent 不应后台观察、判断、甚至替用户按键。未来若要支持接管已有命令，应显式调用 `spawn_cli_agent { task, attach_to_block_id }`，而不是自动发生。

**context 边界**（已在 [HTML §8.3](file:///Users/bytedance/Documents/crest/docs/agent-dual-mode-design.html) 敲定）：
...
```
主 agent (PaneHarness)
   │  tool_call: spawn_cli_agent { task, cwd }
   ▼
...
### 2.0 架构选型结论：方案 A + fallback（已定）

**跨进程 emain↔Go↔renderer 的 RPC 编排（方案 A）是本设计的落地路线**，理由与被否方案如下：
...
**唯一真实痛点 = renderer 不可用**。解法是给 `pty_read` 加 fallback（下方决策 3 的 `mode:"auto"` 即此 fallback 的落地形态）：
...
- 降级下全屏 TUI 会失真，但这是**退化而非不可用**（类比 SSH 断线重连后看到的历史也非完整屏幕）。

> 这里统一了一处早期表述矛盾：方案 A 的原始草稿是"pty_read 一律 renderer-first、失败降级 transcript"；后续几轮把默认翻转为 transcript-first。最终定稿用 `mode:"auto"` 收敛为——**默认 transcript-first，仅在 alt-screen/TUI 时 renderer-first，且 renderer 失败一律降级 transcript**。主/备关系不再随命令类型摇摆，由 `mode` 显式决定。

### 复用（零改动）
- [`AgentHarness`](file:///Users/bytedance/Documents/crest/emain/agent/harness/agent-harness.ts)：CLI subagent 就是**第二个 `AgentHarness` 实例**，独立 session、独立 context。其 turn / tool-call 循环、`setActiveTools`、tool_call gate 全部复用。
...
> **当前 crest 完全缺失 subagent / sub-task 委派机制**（已确认）。`spawn_cli_agent` 是第一个引入嵌套 harness 的地方，实现时注意信号透传（主 agent abort → subagent abort）。

### 2.4 关键实现决策（跨进程 / PTY 托管）

这是本设计最核心的约束：**agent harness 跑在 emain（Node main 进程），但 PTY 执行与终端渲染跨在 frontend renderer + Go blockcontroller**。现有 [`bash` 工具用 node `spawn` 直接在 main 侧起子进程](file:///Users/bytedance/Documents/crest/emain/agent/tools/bash.ts#L59-L119)，输出会作为 agent toolResult 文本显示，但它没有真实 PTY、不能中途交互、也要求命令自行结束。这对会自然结束的一次性命令没问题，但长运行/交互式命令必须在 UI 可见并可被用户接管（决策 4 要完整 PTY 交互），不能沿用 bash 的偷跑路子。以下六个决策据此展开。
...
**采用 A**。这与既定设计"用户直跑 vs agent 发起，用 `source` 标记区分同一套 tool_call/toolResult"天然吻合——subagent 发起的命令就是 `source:"agent"` 的 cmdblock。控制器生命周期见 [`blockcontroller-lifecycle.md`](file:///Users/bytedance/Documents/crest/aiprompts/blockcontroller-lifecycle.md)。

#### 决策 2：pty_write 走 ControllerInput RPC，不碰 node 子进程

写输入复用现成的 [`ControllerInputCommand`](file:///Users/bytedance/Documents/crest/frontend/app/term/terminal-model.ts#L434-L443) → Go [`SendInput`](file:///Users/bytedance/Documents/crest/pkg/blockcontroller/blockcontroller.go)。`BlockInputUnion` 已支持三类输入，正好覆盖 `pty_write` 的三种 mode：
...
> **决策点**：emain 已有 [`ElectronWshClient`](file:///Users/bytedance/Documents/crest/emain/emain-wsh.ts#L13-L158) / `RpcApi.*Command` 这条 wshrpc 通道；本设计不是从零打通跨进程通信，而是在现有通道上补齐"agent 发起 cmd block"与"向 block controller 写输入"这两类能力。

#### 决策 3：pty_read 默认读后端 transcript tail，renderer screen snapshot 只是增强

CLI subagent 多数时候不需要 Warp-style 当前屏幕，只需要"命令最近输出了什么"。因此 `pty_read` 的默认来源应是 Go 后端已有的 PTY 输出（`BlockFile_Term` / cmdblock output tail），而不是 renderer grid。
...
**采用分层读取**：`pty_read(mode:"auto")` 先读 transcript tail；只有检测到 alt-screen / TUI，或 subagent 明确要求 `mode:"screen"` 时，才问 renderer 要 screen snapshot。renderer 不可用时返回 transcript tail 并标记 `degraded:true`。

这把 renderer 从基础依赖降级为增强能力。
...
> 这样"是不是 TUI"这个判断在 Go 一侧就有权威答案，`auto` 不需要靠"控制序列多不多"之类的启发式，也不需要 renderer 参与判定。`is_alt_screen_active` 字段直接来自这个后端状态位；仅当真的走了 screen snapshot 分支时，才用 renderer 的 [`Block.altScreen.active`](file:///Users/bytedance/Documents/crest/frontend/app/term/engine/block.ts#L270-L276) 校准。
>
> 已知边界：`detectAltScreen` 的注释说明——若 `?1049h/l` 序列**恰好跨两个 chunk 被劈开**会漏检，但 app 通常会在下一次 toggle 重新发出。对 subagent 影响可接受（最坏一次 read 走错分支），必要时可在 emain 侧对 tail 内容做二次 `?1049` 兜底扫描。

##### 3b. transcript tail 的数据必须稳定可用（避开循环文件坑）

`BlockFile_Term` 是**循环文件**：shell 重启会 reset、写入超过 `MaxSize` 会 wrap（见 [`filestore` 的 `DataStartIdx`/`DataLength`](file:///Users/bytedance/Documents/crest/pkg/filestore/blockstore.go#L66-L83)）。历史上正因直接按绝对 offset 切这个共享文件，导致 shell 重启后 offset 串位、渲染出别的会话的字节——[migration 000013](file:///Users/bytedance/Documents/crest/db/migrations-wstore/000013_cmdblock_output.up.sql) 为此给每个 cmdblock 加了独立的 `output_data` BLOB 快照。transcript tail 必须绕开同样的坑，按命令**是否已结束**分两条取法：

| 命令状态 | 数据来源 | 稳定性保证 |
| --- | --- | --- |
| **运行中**（subagent 主循环里最常见） | 从 `BlockFile_Term` **尾部**倒读最近 N 行：`ReadFile` 拿 `(offset, data)` 后从末尾切；用 [`filestore.ReadAt`](file:///Users/bytedance/Documents/crest/pkg/filestore/blockstore.go#L380-L388) 按 `DataStartIdx()`~`Size` 的**有效区间**读，绝不用陈旧绝对 offset | 只读"当前有效窗口的尾部"，天然对 wrap/reset 免疫；读的是最新字节，正是 subagent 想要的"最近输出" |
...
落地要点（写进步骤 2b）：
- emain 侧的 output-tail RPC **不接受 caller 传绝对 offset**，只接受 `max_lines` / `max_bytes`，offset 计算全在 Go 端基于当前 `DataStartIdx()`~`Size` 完成，避免把循环文件的坑暴露给 subagent。
- 返回结构里的 `exit_code` / `is_running` 来自 controller 的 `ProcStatus`/`ProcExitCode`（见 [`shellcontroller` wait 循环](file:///Users/bytedance/Documents/crest/pkg/blockcontroller/shellcontroller.go#L525-L590)），让 subagent 能可靠判断"命令是否已结束"从而切到 `output_data` 快照来源。
- tail 文本按需去 ANSI（subagent 读的是语义文本，不是渲染帧）。

> 结论：transcript tail 的"稳定可用"不是新造轮子——运行中走 filestore 有效区间尾读、结束后走 `output_data` 快照，都是复用 crest 已经踩过坑、已经存在的机制。
...
### `pty_read` 参数 / 返回
参数含 `delay`（读之前等多久，让输出落定，对应 Warp `ShellCommandDelay` [action/mod.rs L790-793](https://github.com/warpdotdev/warp/blob/main/crates/ai/src/agent/action/mod.rs#L790-L793)）和 `mode`：
...
> **不进主 context**：无论 transcript tail 还是 screen snapshot，都只喂给 CLI subagent 当前循环。主 agent 只收到 CLI subagent 总结。

### 输出来源（crest 侧）
默认 transcript tail 来自 Go 后端已有 PTY 输出链路：[`shellcontroller`](file:///Users/bytedance/Documents/crest/pkg/blockcontroller/shellcontroller.go#L525-L590) 读 PTY bytes 并 append 到 `BlockFile_Term`。screen snapshot 则来自已渲染的 frontend grid（逐行拼 `Cell.char`），不重新解析字节流。第一阶段只要求 transcript tail 跑通；screen snapshot 是 TUI 增强。
...
- **输入**：`spawn_cli_agent { task: string, cwd: string }`。`task` 是目标描述（"启动 dev server 并确认监听在 3000 端口"），不是命令行。
...
| 步骤 | 文件 / 位置 | 动作 | 依据 |
| --- | --- | --- | --- |
| 1 | `emain/agent/tools/pty-write.ts` `pty-read.ts` `pty-transfer.ts` | 新增三件套（typebox schema + `execute`），围绕已有 `blockId`；`pty_read` 默认 transcript tail | §3 |
...
| 5 | 主 agent 工具注册处 | 把 `spawn_cli_agent` 加入主 agent tools（与 `bash` 并列） | §1 |
| 6 | subagent system prompt | 写 §5 五条要点 | §5 |
| 7 | 测试 | subagent 循环收敛、abort 透传、`pty_read` 原始输出不进主 context、超步移交、pty_write 三 mode 字节正确 | 决策 6 |
